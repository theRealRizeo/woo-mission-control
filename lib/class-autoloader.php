...
class Autoloader {
	...
	public function autoload( $class ) {
		...
		// If its not a MissionControl class, exit now.
		if ( ! preg_match( '/^MissionControl/', $class ) ) {
			return false;
		}
		...
		if ( ! empty( $class_parts ) && 'MissionControl' === $class_parts[0] ) {
		...
		if ( ! empty( $class_string ) ) {
			...
			$filename = apply_filters( 'missioncontrol_class_file_override', $basedir . 'class-' . $class_string . '.php', $class );
